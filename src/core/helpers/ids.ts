...
export function generatePrefixId(
  prefix = "tu",
  length = DEFAULT_PREFIX_ID_LENGTH
): string {
  return `${prefix}_${generateId(length)}`;
}

export function generateMessageId(length = DEFAULT_PREFIX_ID_LENGTH) {
  return generatePrefixId("message", length);
}
